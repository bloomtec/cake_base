...
<?php echo $this->Form->create('Address',array('action' => 'edit'));?>
	...
	<?php
		echo $this -> Form -> input('Address.id', array('options' => $addressList,'label'=>__('Address Name',true)));
		echo $this -> Form -> hidden('Address.user_id', array('value' => $addresses[0]['Address']['user_id']));
		echo $this -> Form -> input('Address.name', array('required' => 'required', 'value' => $addresses[0]['Address']['name'],'id'=>'name'));
		echo $this -> Form -> input('Address.country_id', array('required' => 'required','options' => $countries, 'selected' => $address['Address']['country_id'],'id'=>'country'));
		echo $this -> Form -> input('Address.city_id', array('required' => 'required','options' => $cities, 'selected' => $addresses[0]['Address']['city_id'],'id'=>'city'));
		echo $this -> Form -> input('Address.zone_id', array('required' => 'required','options' => $zones,'selected'=>$addresses[0]['Address']['zone_id'],'id'=>'zone'));
		echo $this -> Form -> input('Address.address', array('required' => 'required', 'value' => $addresses[0]['Address']['address'],'id'=>'address','type'=>'text'));	
	?>
	</fieldset>
<?php echo $this->Form->end(__('Submit', true));?>

<?php endif;?>
<h1><?php __('Add New Address')?></h1>
<?php echo $this->Form->create('Address',array('action' => 'add'));?>
	...
	<?php
		...
		echo $this -> Form -> input('Address.zone_id', array('required' => 'required','options' => $zones));
		echo $this -> Form -> input('Address.address', array('required' => 'required','type'=>'text'));
	?>
	</fieldset>
<?php echo $this->Form->end(__('Submit', true));?>
</div>
