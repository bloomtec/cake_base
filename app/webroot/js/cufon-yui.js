...
var Cufon = (function() {
	...
	var DOM = api.DOM = {
		...
		ready : (function() {
			...
			if (document.addEventListener) {
				...
				window.addEventListener('pageshow', perform, false); // For
																		// cached
																		// Gecko
																		// pages
			}
		...
		strict : (function() {
			var doctype;
			// no doctype (doesn't always catch it though.. IE I'm looking at
			// you)
			...
			// IE<9, firstChild is the doctype even if there's an XML
			// declaration
	...
	var CSS = api.CSS = {
		...
		// @see
		// http://msdn.microsoft.com/en-us/library/system.windows.fontstretches.aspx
		...
		whiteSpace : (function() {
			...
			return function(text, style, node, previousElement, simple) {
				if (simple)
					return text.replace(wsStart, '').replace(wsEnd, ''); // @fixme
																			// too
																			// simple
				if (previousElement) {
	...
	CSS.ready = (function() {
		...
		function isSheetReady(sheet, media) {
			...
			try {
				...
				if (rules) {
					...
					// in standards-conforming browsers cssRules contains
					// @-rules.
					// Chrome 1.0 weirdness: rules[<number larger than .length -
					// 1>]
		...
		function allStylesLoaded() {
			// Internet Explorer's style sheet model, there's no need to do
			// anything
		...
		DOM.ready(function() {
			// getComputedStyle returns null in Gecko if used in an iframe with
			// display: none
			if (!hasLayout)
	...
	function Font(data) {
		...
		this.spacing = function(chars, letterSpacing, wordSpacing) {
			...
			while (chr = chars[++i]) {
				...
				w = glyph.w;
				if (isNaN(w))
					w = +this.w; // may have been a String in old fonts
				if (w > 0) {
					w += letterSpacing;
	...
	api.replace = function(elements, options, ignoreHistory) {
		...
		if (!options.engine)
			return api; // there's no browser support so we'll just stop here
		if (!initialized) {
			CSS.addClass(DOM.root(), 'cufon-active cufon-loading');
			CSS.ready(function() {
				// fires before any replace() calls, but it doesn't really
				// matter
...
Cufon
		.registerEngine(
				...
				(function() {
					...
					// Combined with getFontSizeInPixels it also works with
					// relative units.
					...
					return function(font, text, style, options, node, el,
						...
						if (redraw) {
							...
							// ie6, for some reason, has trouble rendering the
							// last VML element in the document.
							// we can work around this by injecting a dummy
						...
						while (chr = chars[++i]) {
							...
							if (redraw) {
								...
								while (shape.firstChild)
									shape.removeChild(shape.firstChild); // shadow,
																			// fill
							} else {
							...
							// it's important to not set top/left or IE8 will
							// grind to a halt
							...
							if (shadows) {
								// due to the limitations of the VML shadow
								// element there
								// can only be two visible shadows. opacity is
								// shared
								// for all shadows.
		...
		.registerEngine(
				...
				(function() {
					...
					return function(font, text, style, options, node, el) {
						...
						if (!jumps.length)
							return null; // there's nothing to render
						...
						function renderText() {
							...
							while (chr = chars[++i]) {
								...
								if (glyph.d) {
									g.beginPath();
									// the following moveTo is for Opera 9.2. if
									// we don't
									// do this, it won't forget the previous
									// path which
